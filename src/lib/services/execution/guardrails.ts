...
const processedKeys = new Map<string, string>(); // key → orderId or "pending"
const MAX_IDEMPOTENCY_CACHE = 1000;

export function checkIdempotency(key: string): GuardrailResult {
  if (!key) return { passed: true }; // no key = no dedup (will be generated by caller)
  const existing = processedKeys.get(key);
  if (existing && existing !== "pending") {
    return {
      passed: false,
      error: `Duplicate order: idempotencyKey ${key} already processed (order ${existing})`,
      errorCode: "DUPLICATE_ORDER",
    };
  }
  // "pending" keys are cleaned up on failure — allow retry
  if (existing === "pending") {
    return { passed: true }; // allow retry of a failed pending key
  }
  return { passed: true };
}
...
/** Remove a pending key (called when guardrails or execution fail) */
export function clearPendingKey(key: string): void {
  if (processedKeys.get(key) === "pending") {
...
export function checkDuplicateOrder(
  ...
  // Check if an identical order was placed in the last 30 seconds
  const windowAgo = new Date(Date.now() - 30_000).toISOString();
...
export function runGuardrails(
  ...
  // Input validation
  if (!request.instrument || request.instrument.trim() === "") {
    return { passed: false, error: "Missing instrument", errorCode: "UNKNOWN" };
  }
  if (!request.quantity || request.quantity <= 0 || !Number.isInteger(request.quantity)) {
    return { passed: false, error: `Invalid quantity: ${request.quantity}`, errorCode: "UNKNOWN" };
  }
  if (request.type === "limit" && (request.price == null || request.price <= 0)) {
    return { passed: false, error: "Limit order requires positive price", errorCode: "UNKNOWN" };
  }
  if ((request.type === "stop" || request.type === "stop_limit") && (request.stopPrice == null || request.stopPrice <= 0)) {
    return { passed: false, error: "Stop order requires positive stopPrice", errorCode: "UNKNOWN" };
  }
